...
SimulateFinalsGame <- function() {
  ...
  if (this.final == "GF") {
    finals[this.final, "ground"] <- "M.C.G."
  } else {
...
SimulateFinalsElo <- function(ladder.data) {
  ...
  team.home <-
    ladder.data %>%
    filter(ladder.posn == 5) %>%
    .[["team"]]
  
  team.away <- 
    ladder.data %>% 
    filter(ladder.posn == 8) %>% 
    .[["team"]]
  ...
  this.final <- finals$rnd == "EF2"
  
  team.home <-
    ladder.data %>%
    filter(ladder.posn == 6) %>%
    .[["team"]]
  
  team.away <- 
    ladder.data %>% 
    filter(ladder.posn == 7) %>% 
    .[["team"]]
  ...
  this.final <- finals$rnd == "QF1"
  
  team.home <-
    ladder.data %>%
    filter(ladder.posn == 1) %>%
    .[["team"]]
  
  team.away <- 
    ladder.data %>% 
    filter(ladder.posn == 4) %>% 
    .[["team"]]
  
  ??? <- SimulateFinalsGame()
  
  # Qualifying final 2: 2nd vs 3rd
  this.final <- finals$rnd == "QF2"
  
  team.home <-
    ladder.data %>%
    filter(ladder.posn == 2) %>%
    .[["team"]]
  
  team.away <- 
    ladder.data %>% 
    filter(ladder.posn == 3) %>% 
    .[["team"]]
